...
hAnt = 0 # variable que lleva la hora anterior para detectar flanco
	...
	if topic == "caja":
		...
		if payload["dia"] == "monday":
			...
			# Si no hay puerta abierta en la caja no hace nada.
				...
				# Comprueba si la puerta abierta se ha sido por la mañana
				if time.localtime().tm_hour <= 12:
					cur.execute("UPDATE customers SET Ding = 1 WHERE ID = {}".format(cID))
					con.commit()
				# Comprueba si la puerta abierta se ha sido a la comida	
				elif (time.localtime().tm_hour > 12) and (time.localtime().tm_hour < 16):
					cur.execute("UPDATE customers SET Cing = 1 WHERE ID = {}".format(cID))
					con.commit()
				# Comprueba si la puerta abierta se ha sido por la noche		
				else:
					cur.execute("UPDATE customers SET Ning = 1 WHERE ID = {}".format(cID))
					con.commit()
...
if __name__ == '__main__':
	...
	while True:
		...
		if ((hora == 15) and (hAnt != 15 )):
			try:
				cur.execute("SELECT ID,umbralM FROM customers")
				conv = cur.fetchall()
				for i in conv:
					ID,umbral = i
		...
		if ((hora == 23) and (hAnt != 23)):
			try:
				cur.execute("SELECT ID,umbralT FROM customers")
				conv = cur.fetchall()
				for i in conv:
					ID,umbral = i
		...
		if(hora != hAnt):
			...
			if hora <= 12:
				try:
					cur.execute("SELECT ID,Ding FROM customers WHERE (({} >= Desayuno) AND ({} > Desayuno)) AND (Ding = 0)".format(hora,hora-1))
					conv = cur.fetchall()
					for i in conv:
						ID1,DING = i
						print(ID1,"Desayuno\n")
			...
			if (hora > 12) and (hora <= 16):
				try:
					cur.execute("SELECT ID,Cing FROM customers WHERE (({} >= Comida) AND ({} > Comida)) AND (Cing = 0)".format(hora,hora-1))
					conv = cur.fetchall()
					for i in conv:
						ID1,CING = i
						print(ID1,"Comida\n")
			...
			if (hora >16) and (hora <= 23):
				try:
					cur.execute("SELECT ID,Ning FROM customers WHERE (({} >= Noche) AND ({} > Noche)) AND (Ning = 0)".format(hora,hora-1))
					conv = cur.fetchall()
					for i in conv:
						ID1,NING = i
						print(ID1,"Noche\n")
		...
		if(hora != hAnt):
			try:
				cur.execute("SELECT ID,Ning FROM customers WHERE (Desayuno = -1 AND Ning = 1) OR (Comida = -1 AND Cing = 1) OR (Noche = -1 AND Ning = 1)".format(hora,hora-1))
				conv = cur.fetchall()
				for i in conv:
					ID1,NING = i
