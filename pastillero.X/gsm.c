...
{ 
    ...
    if(i==10)   // No arranca,fuera de secuencia..
    {
        // Pulso encendido.
        PWX_LAT = 0;
        DELAY_milliseconds(1500);
...
// obtiene la hora de la celda.
void getHora(char *fecha,int maxlen)
{
    waitIni(fecha,maxlen);
    exeuno(&gethora,fecha,maxlen);      // Hora local de la celda. 
}
...
void gsmoff(char *linea,int maxlen)
{
    ...
    DELAY_milliseconds(2000);
    // comprobamos apagado..
    if(waitIni(linea,maxlen))   // aun encendido.. fuera de secuencia
    {
        // Apagamos GSM.(pulso de apagado)
        PWX_LAT = 0;
        DELAY_milliseconds(1500);
